...
string Base64::encode(const string& src, int line_len) {
    ...
    for (int i=0; i<(src_len / 3); i++) {
        ...
        tmp[1] = *src_buff++;
        tmp[2] = *src_buff++;
        str += _encode_table[(tmp[0] & 0xFC) >> 2];
        str += _encode_table[((tmp[0] << 4) | (tmp[1] >> 4)) & 0x3F];
        str += _encode_table[((tmp[1] << 2) | (tmp[2] >> 6)) & 0x3F];
        str += _encode_table[tmp[2] & 0x3F];
        len += 4;
    ...
    case 1:
        tmp[0] = *src_buff++;
        str += _encode_table[(tmp[0] & 0xFC) >> 2];
        str += _encode_table[(tmp[0] & 0x03) << 4];
        str += "==";
        break;
    case 2:
        tmp[0] = *src_buff++;
        ...
        str += _encode_table[(tmp[0] & 0xFC) >> 2];
        str += _encode_table[((tmp[0] << 4) | (tmp[1] >> 4)) & 0x3F];
        str += _encode_table[((tmp[1] & 0x0F) << 2)];
        str += "=";
        break;
    default:
        break;
...
string Base64::decode(const string& src) {
    ...
    for (int k=0, i=0, value=0; i<src_len; i++) {
        ...
        if ('\r' != u_char && '\n' != u_char) {
            switch (k) {
            case 0:
                value = _decode_table[u_char] << 18;
                break;
            case 1:
                value += _decode_table[u_char] << 12;
                str += (value & 0x00FF0000) >> 16;
                break;
            case 2:
                if ('=' != u_char) {
                    value += _decode_table[u_char] << 6;
                    str += (value & 0x0000FF00) >> 8;
                }
                break;
            case 3:
                if ('=' != u_char) {
                    value += _decode_table[u_char];
...
const char Base64::_encode_table[] =
    "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";

const char Base64::_decode_table[] = {
